...
**2026-08-08 audit — two items on this list were STALE, re-verified against code:**
- Batch RECALL is BUILT, not pending: `PharmacyController` + routes + panel
  `src/modules/pharmacy/`. Stock movements record their batch allocations at the
  moment of consumption, which is what makes a recall answerable at all.
- `dosage_form` EXISTS (StoreProductRequest) but it is the FORM — tablet/syrup —
  used for generic-alternative matching, NOT patient directions.

**Still genuinely missing (verified 2026-08-08):**
- **Per-item dispensing DIRECTIONS ("1 tablet twice daily after meals").** Rx
  capture is SALE-level only — `2026_07_22_000002_add_prescription_to_sales.php`
  adds prescription_number / prescriber_name / patient_name / prescription_notes
  to `sales`. A grep for directions|sig|instructions across ALL migrations
  returns nothing. Real prescriptions carry different directions per medicine,
  so one free-text note on the whole sale cannot express them and nothing can be
  printed on the label. Fix: nullable `directions` on sale_items + a per-line
  field in the Rx modal + receipt/label print.
- Near-expiry NOTIFICATION (mirror the low-stock Notification dedupe path in
  InventoryService) — currently visibility only, no push/alert record.
- CSV bulk import of medicine stock still creates no batch (noted above).

Re-verified CORRECT and worth not re-auditing: FEFO ordering
(`orderByRaw('expiry_date IS NULL, expiry_date')`), expired lots excluded from
allocation, and the STOCK_EXPIRED fence (`sellable = current - expired`, under
lockForUpdate) — InventoryService.
