...
metadata: 
  ...
  modified: 2026-07-29T11:38:57.172Z
---
...
**Remaining retail gaps:** capture serials at PO-RECEIVE / opening stock (currently only at point of sale — a shop can't pre-register IMEIs into stock); per-serial RETURN matching (return currently frees serials only on whole-sale cancel/full-refund, not when picking which specific serial came back on a partial return); serial on CSV import. See [[shopos-audit-backlog]]. Retail was ~20% (warranty/IMEI/serial the main hole) — serialized selling now closes the biggest piece. Payment/launch context in [[shopos-payments-status]].
