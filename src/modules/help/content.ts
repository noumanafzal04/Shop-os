...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  // ── Selling ───────────────────────────────────────────────────────
  {
    ...
    body: [
      ...
      { type: "warn", text: "Orders split by how they are handed over, not by where they came from. A PICKUP order collected at the counter goes into your drawer, because that is where the customer paid — the same for an item somebody reserved online and then came in for. A DELIVERY order does not, even when it is cash on delivery: the rider still has the goods and the money, and a till that expected those notes would leave you counting short with nothing to explain. Record the rider's cash as a paid-in on the shift when they hand it over." },
  ...
  },
  {
    ...
    body: [
      ...
      { type: "steps", items: [
        ...
        "Paste the chart: the depth in millimetres, then the litres it holds at that depth. Two columns, in whatever your table uses to separate them \u2014 a tab out of Excel, commas out of a CSV, spaces out of a PDF.",
        "Save. The close screen now asks for millimetres on that tank and works the litres out itself.",
      ] },
      { type: "note", text: "Any line it could not read as two numbers is listed with its line number before you save, so a paste that went wrong is visible rather than quietly short. A depth listed twice is named too \u2014 the second one is not silently used." },
      { type: "note", text: "Between two depths on your chart, the litres are read straight between them, the same way you would reading between two lines of the printed table. A reading BEYOND the chart is refused rather than guessed at: either the stick was mis-read or that is not this tank\u2019s chart, and both are worth stopping for." },
      { type: "warn", text: "Loading a chart is a setup job, not a shift job \u2014 the person dipping a tank at 2am does not get to redefine what its depths mean. Until a chart is loaded the tank is still dipped in litres exactly as before, and clearing the chart puts it back." },
  ...
  },
  {
    ...
    body: [
      ...
      { type: "p", text: "Every completed sale, newest first. Search by receipt number, customer or amount, then narrow it: pick a date range by name (Today, Last 7 days, This month, or any two dates you choose), and filter by status, how it was paid, where it was rung — the till, the phone, an online order — and, where your shop records it, who served the customer." },
      { type: "note", text: "Whatever the list is showing is what Export CSV writes. Narrow it to one week of card sales and the file has one week of card sales in it — there is no separate set of options to fill in twice." },
      { type: "p", text: "Every filter you have set is listed above the table, each with its own ✕. Nothing is ever narrowed by something you cannot see." },
  ...
  },
  {
    ...
    keywords: ["rider", "delivery", "driver", "dispatch", "assign", "app", "cash", "settle", "rider id", "code"],
    body: [
      { type: "p", text: "The people who take orders out. Add them here, then assign an order to one when it is ready to leave." },
      { type: "h", text: "Two kinds of rider" },
      { type: "p", text: "ADD YOUR OWN RIDER for somebody who works for you and has no app \u2014 a name and a phone number. Nothing changes for them: you assign the order and you move it through Preparing, On the way and Completed yourself." },
  ...
  },
  {
    ...
    body: [
      ...
      { type: "h", text: "What is actually saved on the device" },
      { type: "keys", items: [
        ["Products", "Every sellable item with its price, discount price, wholesale price and quantity breaks, its tax, its stock figure, its sizes and its pack sizes (single, box, carton)."],
        ["Barcodes", "Every code, including the ones on packs and sizes — this is what lets scanning work with the line down."],
        ["Categories", "So the till can still be browsed, not only searched."],
        ["Customers", "Name, phone and which price group they are in. Nothing else — no addresses, no history."],
        ["Prices & tax rules", "Customer group discounts, tax groups, and the promotions that are safe to work out on the device."],
        ["Shop settings", "Your till settings, so the POS behaves the same offline as on."],
        ["The open shift", "So a till that reloads during an outage can carry on selling into the drawer it was already selling into."],
      ]},
      { type: "note", text: "And everything you ring while offline: sales and drawer events wait on the device until the connection returns, along with the slip numbers already given out. Those are never cleared by a sync." },
      { type: "warn", text: "NOT saved: product photos, past sales and invoices, and reports. A till can sell offline; it cannot look up last Tuesday's takings offline." },
      { type: "note", text: "If this device is ever signed into a DIFFERENT shop, everything above is cleared automatically and the new shop's own list is fetched — one device never shows two shops' products. Anything still waiting to be sent is kept, because it is money that already crossed a counter." },
      ...
      { type: "keys", items: [
        ...
        ["Holding a sale", "Yes, with one honest limit. Normally a parked ticket is shared across every counter, and picking one up is a locked step so two lanes cannot ring the same basket. With no line there is nothing to share it through, so the ticket is parked on THAT TILL: park it, start the next customer, and pick it up at the same counter. The till says so before it parks, again after, and marks the row 'this till only', so you can tell the customer which counter to come back to. Tickets from other lanes are not shown while the line is down — the list says that rather than pretending to be empty. They are not sent up when the connection returns: a basket that had already been rung would come back to the shop as a ticket to ring again."],
      ...
      { type: "keys", items: [
        ...
        ["Shifts that ran with no server", "A drawer can be opened, used and counted with the line down, and two rules can only be broken that way: the lane was already held by somebody who got back online first, or the cashier already had a shift open elsewhere. Neither is corrected on arrival — a counted drawer must not be left belonging to nothing — so the conflict is named here for you to sort out. A shift still marked open days after it arrived is the one to look at first: that is a drawer nobody has counted."],
      ...
      { type: "warn", text: "One thing this screen CANNOT tell you, and the till says instead. A sale the server refused outright never reaches us, so it can never appear in a report we build. It happens when the tablet's copy of your stock was out of date — it thought there were three on the shelf and there were none — and the sale was rung, paid for and taken away before the connection came back to say no. The till itself carries a red strip for these, with a 'See which' button that names each slip number, the amount and exactly what the server objected to. Deal with them the same morning: the cash is in your drawer with no sale against it, so your drawer will read OVER by that amount until you ring them again or correct what was wrong." },
  ...
  },
  {
    ...
    body: [
      ...
      { type: "note", text: "Filter by what you are looking for rather than scrolling. Credit limits, tax rates, coupons and staff each have their own filter, and the date control narrows it by name \u2014 Yesterday, Last 7 days, or any two days you pick \u2014 down to the week something went wrong. Whatever you have set is listed above the table, each with its own \u2715." },
